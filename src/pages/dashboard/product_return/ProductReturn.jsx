...
const ProductReturn = () => {
  ...
  const handleConfirmOrder = async () => {
    ...
    const payload = {
      ...
      voucherNo: items[0]?.voucherCode || "N/A",
      customerId: items[0]?.customerId || 1,
      // totalQuantity: items.reduce((sum, i) => sum + (Number(i.qty) || 1), 0),
      note: items[0]?.note || "N/A",
      // Items array
      products: items.map((i) => ({
        productId: Number(i.productId) || null,
        quantity: Number(i.quantity) || 1,

        paymentCatId: Number(i.paymentCatId) || null, // 0 → null
        amount: Number(i.cash) || null, // 0 → null

        shweChainKyat: Number(i.shweChainKyat) || 0,
        shweChainPae: Number(i.shweChainPae) || 0,
        ShweChainYway: Number(i.ShweChainYway) || 0,
        shweChainGram: Number(i.shweChainGram) || 0,

        alyautKyat: Number(i.alyautKyat) || 0,
        alyautPae: Number(i.alyautPae) || 0,
        alyautYway: Number(i.alyautYway) || 0,
        alyautGram: Number(i.alyautGram) || 0,

        // discountKyat: Number(i.discountKyat) || 0,
        // discountPae: Number(i.discountPae) || 0,
        // discountYway: Number(i.discountYway) || 0,
        // discountGram: Number(i.discountGram) || 0,
  ...
  const handleDeleteCashies = (idx) =>{
    dispatch(resetCash(idx))
  }
                    ...
                    return (
                      <tr key={`cashie-${idx}`}>
                        ...
                        <td className="border p-1" colSpan={11}>
                          ငွေပမာဏ: {Number(c.cash || 0).toLocaleString()} Ks
                        </td>
                        ...
                        <td className="border p-1">
                        <Button
                          onClick={() => handleDeleteCashies(idx)}
                          className="border p-5 bg-red-400 text-black hover:bg-red-500 hover:text-white"
                        >
                          X
                        </Button>
