...
const Damage = () => {
  ...
  quantity: Number(i.quantity) || 1, // 0 မဟုတ်အောင် default 1
  ...
  paymentMoney: (cashies || []).map((c) => ({
    paymentCatId: Number(c.paymentCatId) || null, // 0 → null
    amount: Number(c.cash) || null, // 0 → null
  })).filter(pm => pm.paymentCatId && pm.amount), // required field filter
}))
