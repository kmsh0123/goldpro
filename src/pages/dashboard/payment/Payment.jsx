...
import { useDeletePaymentMutation, useGetPaymentQuery } from "@/feature/api/paymentApi/paymentApi";
    ...
    <div className="space-y-4 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4 bg-white rounded-lg border">
        ...
        <Button className="bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg">
          <Link to="/payment/payment-list/create" className="flex items-center gap-2">
            + Create New Payment
          </Link>
        </Button>
      </div>
      ...
      <div className="bg-white rounded-lg border shadow-sm">
        <PaginatedTable
          columns={["No.", "Created Date", "Category Name","Amount","Actions"]}
          ...
          renderRow={(item, index) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="py-3 px-4 text-center font-medium">{item.amount}</td>
