...
import { useCreateProductMutation, useProductDetailQuery, useUpdateProductMutation } from "@/feature/api/inventory/productApi";
...
const UpdateProduct = () => {
  ...
  const [productUpdate, { isLoading: ProductLoading }] = useUpdateProductMutation();
  ...
  const {data : ProductData, isLoading} = useProductDetailQuery(id);
  ...
  useEffect(() => {
    if (ProductData?.data) {
      ...
      reset({
        ...
        shweKyat: p.shwe_chain_kyat || "",
        shwePae: p.shwe_chain_pae || "",
        shweYway: p.shwe_chain_yway || "",
        shweGram: p.shwe_chain_gram || "",
        alyautKyat: p.alyaut_twat_kyat || "",
        alyautPae: p.alyaut_twat_pae || "",
        alyautYway: p.alyaut_twat_yway || "",
        alyautGram: p.alyaut_twat_gram || "",
      });
      // show current image preview
       if (p.image) {
      const fullUrl = p.image.startsWith("http") ? p.image : `${import.meta.env.VITE_API_IMAGEURL}image/${p.image}`;
  ...
  const handleUpdateProduct = async (formData) => {
    try {
      ...
      const { data } = await productUpdate({id,fd});
