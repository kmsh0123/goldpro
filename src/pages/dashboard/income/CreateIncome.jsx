...
import { useCreateIncomeMutation } from "@/feature/api/incomeApi/incomeApi";
import { useGetIncomeCateoryQuery } from "@/feature/api/incomeCategory/incomeCategory";

const CreateIncome = () => {
  const {id} = useParams();
  const nav = useNavigate();
  const [IncomeCreate] = useCreateIncomeMutation();
  ...
  const selectedExpenseCategoryName =
    GetIncomeCateogry?.data?.find((item) => item.id.toString() === selectedIncomeCatgoryId)?.income_category ||
    "";
  ...
  //     ?.name || "";
   ...
   const filteredExpenseCategory = GetIncomeCateogry?.data?.filter((item) =>
    item.income_category.toLowerCase().includes(searchIncomeCategory.toLowerCase())
  );
  ...
  const handleCreateExpense = async (formData) => {
    try {
      ...
      const { data } = await IncomeCreate(payload);
        ...
        {/* <div>
         ...
         <div>
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Popover open={openIncomeCategory} onOpenChange={setOpenIncomeCategory}>
                <PopoverTrigger asChild>
                  <div>
                    <Button
                      type="button"
