...
import { useDeleteIncomeMutation, useGetDetailIncomeQuery, useGetIncomeQuery } from "@/feature/api/incomeApi/incomeApi";
    ...
    <div className="space-y-4 p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-4 bg-white rounded-lg border">
        ...
        <Button className="bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg">
          <Link to="/income/income-list/create" className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg border shadow-sm">
        <PaginatedTable
          columns={["No.", "Income Date", "Income Category Name","Income Amount","Income Category Note", "Actions"]}
          ...
          renderRow={(item, index) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="py-3 px-4 text-center font-medium">{item?.income_for}</td>
              <td className="py-3 px-4 text-center font-medium">{Number(item?.income_amount)?.toLocaleString()}</td>
