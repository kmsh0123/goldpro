...
import { useCreateExpenseMutation } from "@/feature/api/expenseApi/expenseApi";
...
const CreateExpense = () => {
  ...
  const [ExpenseCreate] = useCreateExpenseMutation();
  ...
  const selectedExpenseCategoryName =
    GetExpenseCateogry?.data?.find((item) => item.id.toString() === selectedExpenseCatgoryId)?.expense_category ||
    "";

   const selectedPaymentCategoryName =
    GetPaymentCateogry?.data?.find((item) => item.id.toString() === selectedPaymentCategoryId)?.category_name ||
    "";
  ...
  //     ?.name || "";
   ...
   const filteredExpenseCategory = GetExpenseCateogry?.data?.filter((item) =>
    item.expense_category.toLowerCase().includes(searchExpenseCategory.toLowerCase())
  );

  const filteredPaymentCategory = GetPaymentCateogry?.data?.filter((item) =>
    item.category_name.toLowerCase().includes(searchPaymentCategory.toLowerCase())
  );
  ...
  const handleCreateExpense = async (formData) => {
    try {
      const payload = {
        ...
        expenseAmount: String(formData.expenseAmount), // backend string အနေနဲ့ လိုရင် stringify
      };

      const { data } = await ExpenseCreate(payload);
      console.log("Expense created successfully:", data);
      toast.success("Expense created successfully!");
      reset();
      nav("/expense/expense-list");
    } catch (error) {
      console.error("Error create Expense");
    }
  };
        ...
        <div>
         ...
         <div>
          ...
          <Controller
            ...
            control={control}
            render={({ field }) => (
              <Popover open={openPaymentCategory} onOpenChange={setOpenPaymentCategory}>
                <PopoverTrigger asChild>
                  <div>
                    <Button
                      type="button"
